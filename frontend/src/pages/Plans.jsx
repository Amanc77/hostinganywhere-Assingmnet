...
function PlansSimple() {
  const navigate = useNavigate();
  const [plans, setPlans] = useState([]);

  useEffect(() => {
    const t = setTimeout(() => setPlans(samplePlans), 100);
    ...
    <div className="min-h-screen bg-[#0d1117] text-white">
      <div className="max-w-7xl mx-auto px-6 py-24">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-bold">Choose Your Perfect Plan</h1>
          <p className="text-gray-400 mt-3 text-lg">
            Simple and transparent pricing for your hosting needs
          </p>

          <div className="mt-8 flex justify-center gap-4">
            <button
              onClick={() => navigate("/")}
              className="px-5 py-2 rounded-md border border-gray-700 text-gray-300 hover:bg-gray-800"
            >
              Home
            </button>
            <button
              onClick={() => navigate("/login")}
              className="px-5 py-2 rounded-md bg-indigo-600 hover:bg-indigo-500 text-white"
            >
              Get Started
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className="rounded-xl bg-[#161b22] border border-gray-800 p-8 shadow-lg hover:shadow-xl  transition"
            >
              {plan.name === "Standard" && (
